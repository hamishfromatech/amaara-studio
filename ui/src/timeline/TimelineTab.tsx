...
export function TimelineTab() {
  ...
  const aspect =
    timeline.width > 0 && timeline.height > 0 ? timeline.width / timeline.height : 16 / 9
  const selectedClip =
    ...
    <div className="flex flex-1 flex-col overflow-hidden bg-canvas">
      ...
      {selectedClip && (
        <div className="flex items-center gap-2 border-t border-line-soft px-4 py-1.5 text-xs text-ink-muted">
          ...
          <span>·</span>
          <span>
            {fmtSec(selectedClip.start_s)}–{fmtSec(selectedClip.start_s + selectedClip.duration_s)}
          </span>
          {selectedClip.src && (
            <>
              <span>·</span>
  ...
  return (
    <div className="mb-3 flex items-center gap-3 rounded-lg border border-line-soft bg-panel px-3 py-2">
      ...
      <div
        ...
        aria-valuemin={0}
      >
        <div className="h-full bg-info/30" style={{width: `${ratio * 100}%`}} />
        <div
      ...
      <span className="mono w-20 text-center text-xs text-ink-muted">
        {fmtSec(playheadMs / 1000)}
      </span>
