...
export function RendersTab() {
  ...
  const selected =
    renders.find((r) => r.job_id === selectedId) ??
    renders.find((r) => r.status === 'running') ??
    renders[0] ??
    null
    ...
    <div className="flex-1 overflow-y-auto bg-canvas p-5 text-ink">
      ...
      {renders.length === 0 ? (
        ...
        <>
          ...
          {/* Selected job detail */}
          {selected && (
            <JobDetail job={selected} onCancel={() => void cancelRender(selected.job_id)} />
          )}
        </>
      )}
    </div>
  ...
  return (
    <div
      ...
      className={`queue-row flex cursor-pointer items-center justify-between gap-2 rounded-lg border px-2.5 py-2 transition-colors ${
        selected ? 'border-line-selected bg-panel' : 'border-line-soft bg-panel hover:border-line'
