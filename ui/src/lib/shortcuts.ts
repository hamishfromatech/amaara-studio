...
/** Center tabs reachable by ⌘1..⌘4 (design.md §10 lists chat/timeline/assets/
 * renders; this app has no assets tab, so we map to the first four real tabs). */
const TAB_KEYS: NavId[] = ["home", "timeline", "renders", "models"];
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      switch (k) {
        ...
        case "e": {
          ...
          if (instruction) void useStore.getState().sendPrompt(instruction);
          break;
        }
        case "\\":
          e.preventDefault();
