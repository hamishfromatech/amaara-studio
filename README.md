...
Built as a **Tauri 2 desktop app** (Rust core + React webview). An AI agent — the *harness* — orchestrates everything: it writes scripts and storyboards, generates images, assembles **HyperFrames / Remotion** compositions, and renders video locally. Intelligence comes from **Amaara Cloud** (OpenAI-compatible, remote) or **fully local models** (llama.cpp + stable-diffusion.cpp) — one switch, no code changes.
...
- **Tools** — written once in the Rust control server, exposed to the agent as MCP tools (`amaara-mcp/`, FastMCP) or an a-coder-cli extension: `generate_image`, `render_to_video`, `set_generation_source`, `get_project_state`, `snapshot`, …
...
3. `generate_image` routes to Amaara Cloud or the local `sd-server` depending on your source toggle.
...
- An Amaara API key (entered on first launch; stored in the OS keyring)
...
│                     #   Amaara/SD/render clients, SQLite store, sidecar mgmt
├── ui/               # React SPA (chat, projects, models, timeline, renders)
├── amaara-mcp/        # FastMCP (Python) tool server → proxies to the control server
